...
- 去重置顶：重复复制相同内容时原记录刷新到顶部，不产生冗余条目
- 支持关键词搜索和类型筛选
- 海量历史增量分页加载，长列表依旧流畅
- 键盘导航：方向键选择、回车粘贴
...
- 包含剪贴板、快捷短语两个标签页，悬停即切换
- 分类筛选、滚轮滚动选择，内容切换带丝滑过渡动画
- 玻璃拟态搜索栏：圆形按钮展开为全宽搜索条，实时过滤记录
- 点击即粘贴，无需打开主窗口
- 可在设置中开关

### 系统功能

- **全局快捷键**：自定义唤起主窗口，支持 Ctrl / Alt / Shift / Win 的任意组合（字母、数字、小键盘、F 键），含 Win+V；Windows 下由低级键盘钩子实现
